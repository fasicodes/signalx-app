...
function buildLiquidityRadar(sweep, price) {
  ...
  function plot(list, angles, sideClass) {
    list.forEach((p, i) => {
      ...
      const fillColor = p.swept ? "#4a5568" : (sideClass === "side-sell" ? "var(--short)" : "var(--long)");
      const sweptTag = p.swept ? `<span class="liq-radar-swept-tag">SWEPT</span>` : "";
      legendRows += `
        <div class="liq-radar-legend-row">
          <span class="liq-radar-dot ${sideClass}${sweptClass}"></span>
          <span class="liq-radar-price">${fmtPrice(p.price)} ${sweptTag}</span>
  ...
  return `
    <div class="liq-radar-wrap">
      ...
      <div class="liq-radar-key">
        <span><i class="k-sell"></i>Sell-side (resistance, above price)</span>
        <span><i class="k-buy"></i>Buy-side (support, below price)</span>
        <span><i class="k-swept"></i>Already swept (✕)</span>
      </div>
